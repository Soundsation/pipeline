...
if TYPE_CHECKING:
    from UVR_deprecated import ModelData
try:
    from tos_audio_read import tos_read
    TOS_ENABLED = True
except:
    TOS_ENABLED = False
...
class SeperateMDX(SeperateAttributes):      
    ...
    def load_model(self):
        ...
        if self.is_mdx_ckpt:
            ...
            if self.mdx_segment_size == self.dim_t and not self.is_other_gpu and not FORCE_CONVERT_TO_TORCH:
                ...
                ort_ = ort.InferenceSession(self.model_path, opts, providers=self.run_type)
    ...
    def demix(self, mix, is_match_mix=False):
        ...
        for i in range(0, mixture.shape[-1], step):
            ...
            mix_part = torch.as_tensor([mix_part_], dtype=torch.float32).to(self.device)
    ...
    def run_model(self, mix, is_match_mix=False):
        ...
        return self.stft.inverse(torch.as_tensor(spec_pred).to(self.device)).cpu().detach().numpy()
...
def prepare_mix_gpu(mix, device, tos_client=None):
    
    audio_path = mix

    if not isinstance(mix, np.ndarray):
        if mix.startswith('tos://'):
            assert tos_client is not None, 'tos_client must be provided when using tos://'
            object_path = mix.strip("tos://")
            buffer = tos_read(tos_client, object_path)
            mix = buffer
        if mix.endswith(".mp4"):
            torchaudio.set_audio_backend('ffmpeg')
        if mix.endswith(".mp3"):
            mix, sr = torchaudio.load(mix, format='mp3')
        else:
    ...
    # if mix only has 1 channel, duplicate it to 2 channels
    if mix.shape[0] == 1:
        mix = np.asfortranarray([mix[0], mix[0]])
