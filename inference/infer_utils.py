...
def decode_audio(latents, vae_model, chunked=False, overlap=32, chunk_size=128):
    """
    Decode latent representations into audio waveforms using a VAE model.
    
    Args:
        latents: Latent representations to decode
        vae_model: The VAE model used for decoding
        chunked: Whether to use chunked decoding (for longer sequences)
    ...
    io_channels = 2
    if not chunked:
        # Simple decoding for shorter sequences
        return vae_model.decode_export(latents)
    else:
        # Chunked decoding for longer sequences
        ...
        for i in range(num_chunks):
            ...
            if i < num_chunks - 1:
                ...
                t_end -= ol
                chunk_end -= ol
            # merge decoded chunk into final output
            y_final[:, :, t_start:t_end] = y_chunk[:, :, chunk_start:chunk_end]
        return y_final


def prepare_model(max_frames, device, repo_id="Sondsation/model-base"):
    """
    Prepare and initialize all required models for audio generation.
    
    Args:
        max_frames: Maximum number of frames to process
        device: Device to load models on (CPU/GPU)
        repo_id: Hugging Face repository ID for model weights
        
    Returns:
        Tuple of initialized models: (cfm, tokenizer, muq, vae)
    """
    # Load CFM (Conditional Flow Matching) model
    dit_ckpt_path = hf_hub_download(repo_id=repo_id, filename="cfm_model.pt", cache_dir="./pretrained")
    dit_config_path = "./config/model-1b.json"
    ...
    # Load MuQMuLan model for audio understanding
    muq = MuQMuLan.from_pretrained("OpenMuQ/MuQ-MuLan-large", cache_dir="./pretrained")
    muq = muq.to(device).eval()

    # Load VAE model for audio generation
    vae_ckpt_path = hf_hub_download(
        repo_id="Soundsation/VAE",
...
def get_reference_latent(device, max_frames):
    """
    Generate empty reference latent for song editing (placeholder for future functionality).
    ...
    Returns:
        Zero tensor as placeholder reference latent
    """
...
def get_negative_style_prompt(device):
    """
    Load negative style prompt from a predefined file.
    
    Args:
        device: Device to load tensor on
        
    Returns:
        Tensor containing negative style prompt
    """
...
def get_style_prompt(model, wav_path=None, prompt=None):
    """
    Extract style information from either text prompt or audio file.
    
    Args:
        model: MuLan model for audio/text embedding
    ...
    # If text prompt is provided, use it
    ...
    # Extract 10-second segment from middle of audio
...
def parse_lyrics(lyrics: str):
    """
    Parse lyrics with timestamps into a list of (time, lyric) tuples.
    
    Args:
        lyrics: String containing lyrics with timestamps in format [MM:SS.xx]
        
    Returns:
        List of (time_in_seconds, lyric_text) tuples
    """
    lyrics_with_time = []
    lyrics = lyrics.strip()
...
class CNENTokenizer:
    """
    Tokenizer for Chinese and English text processing.
    
    Handles conversion between text and phonetic tokens.
    """
    def __init__(self):
        """Initialize tokenizer with vocabulary mapping"""
    ...
    def encode(self, text):
        ...
        token = [x + 1 for x in token]  # Offset by 1
        return token

    def decode(self, token):
        """Convert token IDs back to phonetic representation"""
        return "|".join([self.id2phone[x - 1] for x in token])


def get_lrc_token(max_frames, text, tokenizer, device):
    """
    Convert lyrics with timing into token sequences for model input.
    
    Args:
        max_frames: Maximum number of frames
        text: Lyrics text with timestamps
        tokenizer: Tokenizer for text conversion
        device: Device to load tensors on
        
    Returns:
        Tuple of (lyrics_embedding, normalized_start_time)
    """
    lyrics_shift = 0
    sampling_rate = 44100
    ...
    # Filter out lyrics beyond max duration
    lrc_with_time = [
        (time_start, line)
    ...
    # Place tokens at appropriate timestamps
    tokens_count = 0
    last_end_pos = 0
    for time_start, line in lrc_with_time:
        # Convert period to comma within line, add period at end
        tokens = [
